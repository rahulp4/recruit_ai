...
class ResumeParserServiceV1:
    ...
    def parse_resume_with_gemini(self, resume_text):
        ...
        try:
            ...
            logger.info(f"LLM Response after cleaning (first 500 chars): \n{json_string[:500]}...")
        ...
        json_schema_string = """
{
  ...
  "experience": [
    {
      "title": "string (MUST extract job title/designation for this role, e.g., 'Sr Software Engineer', 'Director (Product Dev)', 'Project Engineer', 'Principal Software Engineer')",
      "company": "string (MUST extract company name)",
      "location": "string (omit if not present)",
      "from": "string (01/MM/YYYY) (MUST extract start date)",
      "to": "string (01/MM/YYYY or Present) (MUST extract end date)",
      "description": "string (MUST summarize ALL key responsibilities, achievements, and FULL project details that fall under this specific employment period. If projects are listed under a company, include ALL their details here. Ensure the description is comprehensive for the role.)",
      "technologies": ["string (MUST list ALL technologies explicitly mentioned as used within THIS role or its described projects, look for 'Technology:' labels or lists within the role's description, e.g., 'Java', 'Spring Boot', 'AWS', 'Weblogic', 'EJB', 'XML', 'MySQL')"]
    }
  ],
  "education": [
    {
  ...
  "projects": [
    {
      "name": "string (ONLY for standalone personal projects or academic projects, NOT projects detailed under a specific company experience in the 'experience' section)",
      "from": "string (01/MM/YYYY, if available)",
      "to": "string (01/MM/YYYY or Present, if available)",
      "description": "string (omit if not present)",
      "technologies": ["string"],
      "url": "string (URL, omit if not present)"
    }
  ...
  "certifications": [
    {
      ...
      "date": "string (01/MM/YYYY or descriptive like 'Completed')"
        ...
        prompt_parts.append("**Specific Instructions for \"Experience\" Section (e.g., sections titled \"Experience\", \"Position & Company\", \"Work History\", or words meaning similar to these):**\n")
        prompt_parts.append("- Create a distinct object in the \"experience\" array for EACH unique job role or employment period.\n")
        prompt_parts.append("- **CRITICAL: If the experience is presented in a single, comma-separated line (like a CSV row) such as: `\"Job Title Company Name, Location Dates\",- Description`**\n")
        prompt_parts.append("  - You MUST parse the initial quoted segment (e.g., `\"Lead Backend Engineer CloudNet Inc., Bengaluru Mar 2020 – Present\"`) to extract `title`, `company`, `location`, `from` date, and `to` date.\n")
        prompt_parts.append("  - The text following the comma (e.g., `- Designed microservice architecture...`) belongs in the `description` field.\n")
        prompt_parts.append("- For each employment period:\n")
        prompt_parts.append("    - **MUST Extract the \"Designation\" or job role into the \"title\" field.** Provide a clear, concise title (e.g., \"Lead Backend Engineer\", \"Software Engineer\").\n")
        ...
        prompt_parts.append("Desired JSON Schema:\n")
        prompt_parts.append("```json\n")
        prompt_parts.append(json_schema_string.strip() + "\n") # json_schema_string is already stripped
        prompt_parts.append("```\n\n")
    ...
    def _build_gemini_prompt_v1(self, resume_text):
        ...
        prompt = f"""You are a highly skilled resume parser. Your task is to convert the following software engineer's resume text into a structured JSON object.
          ...
          **Specific Instructions for "Projects" Section (top-level):**
          - Use the top-level "projects" array ONLY for standalone personal projects or academic projects that are NOT detailed as part of a specific company's experience.
